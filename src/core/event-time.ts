...
export function eventDateLabel(value:string):string {
  if(!value)return '';
  return new Intl.DateTimeFormat('fr-CA',{timeZone:eventTimezone,weekday:'long',day:'numeric',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).format(new Date(value));
}
