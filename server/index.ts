import { chatDraftForDiscord } from '../src/core/chat';
import { extractEventFacts } from '../src/core/extract-event-facts';
import { planRequestSchema } from '../src/core/plan-request';
import { chatRequestSchema } from '../src/core/chat';
import { ChatService, chatSources } from './chat';
...
import { capabilities, ExecutionService, keyFor, executionBindings, assertExecutionBinding } from './executors';
...
const store=new Store(); const chat=new ChatService(store); const executor=new ExecutionService(store);
...
app.get('/api/status',(_req,res)=>res.json({authRequired:!!token,liveReady:backboardReady(),sideEffects:process.env.SIDE_EFFECTS_ENABLED==='true',capabilities:capabilities()}));
...
app.use('/api',(req,res,next)=>{
 ...
 if(req.method==='POST'){
  ...
  const kind=req.path==='/plan'||req.path==='/ask'||req.path==='/chat'?'generation':'operation';const key=kind+req.socket.remoteAddress;const entry=limits.get(key)||{at:now,count:0};
...
function context(pick:{provider:Plan['provider'];facts:Plan['facts'];id?:string;requestId:string}):Context{const caps=capabilities();return {...pick,executionBindings:executionBindings(),id:pick.id||randomUUID(),now:pick.provider==='scripted'?'2026-09-19T22:45:00-04:00':new Date().toISOString(),calendarEnabled:pick.provider==='scripted'||caps.calendar,discordEnabled:pick.provider==='scripted'||caps.discord,calendarTarget:pick.provider==='scripted'?'Calendar de démonstration':caps.calendarTarget,discordTarget:pick.provider==='scripted'?'#announcements (simulation)':caps.discordTarget};}
...
app.post('/api/plan',async(req,res)=>{
 ...
 try {
  const facts=extractEventFacts(input.prompt);
  if(input.facts)for(const [key,value] of Object.entries(input.facts))if(value.trim())facts[key as keyof typeof facts]=value;
  const generated=await generatePlan(input.prompt,facts,res.locals.requestId);
...
function stageDiscord(text:string,res:express.Response,source='Message Discord rédigé par l’organisateur'){
 if(!capabilities().discord)return res.status(409).json({error:'Configurez une destination Discord avant de préparer ce message.'});
 const result=normalize({actions:[{type:'post_discord',payload:{channel:'announcements',text:'{{fact.message}}'}}],followupPreview:'Message manuel. Aucun suivi programmé.'},context({provider:'manual',facts:factsSchema.parse({message:text}),requestId:res.locals.requestId}));
 if(!result.ok)return res.status(422).json({error:result.errors.join(' ')});
 store.log({requestId:res.locals.requestId,planId:result.value.id,event:'propose',detail:`${source}, en attente d’approbation`});
 res.json(store.save(reduce(null,{type:'propose',proposal:result.value})));
}
app.post('/api/discord/draft',(req,res)=>{
 const {text}=z.object({text:z.string().trim().min(1).max(2000)}).strict().parse(req.body);
 stageDiscord(text,res);
});
app.post('/api/chat/:id/discord-draft',(req,res)=>{
 const input=z.object({expectedRevision:z.number().int().positive(),turnIndex:z.number().int().nonnegative()}).strict().parse(req.body);
 const conversation=chat.get(req.params.id);
 const text=chatDraftForDiscord(conversation,input.expectedRevision,input.turnIndex);
 stageDiscord(text,res,`Brouillon de conversation ${conversation.id}, réponse ${input.turnIndex+1}, sélectionné par l’organisateur`);
});
app.get('/api/plans/:id',(req,res)=>res.json(store.get(req.params.id)));
const decision=z.object({type:z.enum(['approve','reject','execute','confirm_posted','confirm_not_posted','verify_execution']),actionId:z.string().max(100),expectedHash:z.string().max(30000)}).strict();
...
app.post('/api/plans/:id/ops',async(req,res)=>{
 const op=z.union([decision,edit]).parse(req.body);const id=req.params.id;let plan=store.get(id);if(plan.provider==='scripted')return res.status(409).json({error:'Ce plan de démonstration est archivé. Créez un nouveau plan.'});const requestId=res.locals.requestId;
 if(op.type==='edit'){
 ...
 if(op.type==='verify_execution')return res.json(await executor.verify(id,a.id,op.expectedHash,requestId));
 if(op.type==='confirm_posted'||op.type==='confirm_not_posted'){
  ...
  const receipt={id:`manual-${randomUUID()}`,provider:'manual-confirmation',simulated:false};
 ...
 if((op.type==='approve'||op.type==='execute')&&a.capability==='real'&&process.env.SIDE_EFFECTS_ENABLED!=='true')throw new Error('Actions externes désactivées.');
 if(op.type==='approve'&&a.capability==='real'&&['proposed','failed'].includes(a.status))assertExecutionBinding(a);
...
app.get('/api/chat/context',(_req,res)=>res.json(chatSources));
app.get('/api/chat/:id',(req,res)=>res.json(chat.get(req.params.id)));
app.post('/api/chat',async(req,res)=>{res.json(await chat.send(chatRequestSchema.parse(req.body),res.locals.requestId));});
...
app.use((error:unknown,_req:express.Request,res:express.Response,_next:express.NextFunction)=>{res.status(400).json({error:error instanceof z.ZodError?error.issues.map(issue=>`${issue.path.join('.')||'Requête'} : ${issue.message}`).join(' '):error instanceof Error?error.message:'Erreur inconnue.',requestId:res.locals.requestId});});
const port=Number(process.env.PORT||3002);app.listen(port,host,(error?:Error)=>{if(error){console.error('Relais could not bind its local port. Choose an unused PORT.');process.exit(1);}console.log(`Relais: http://${host}:${port} · ${backboardReady()?'live configured':'live not configured'} · ${token?'token protected':'local development only'}`);});
